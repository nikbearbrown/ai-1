...
![Two side-by-side document panels: the left panel shows a generic AI brief as six uniformly filled sections; the right panel shows the same six-section skeleton with six hollow callout markers in the margin, each connected by a thin leader line to a section that needs institutional knowledge — the same surface, holes underneath.](images/01-what-ai-plus-one-is-fig-02.png)
*Figure 1.2 — Generic brief vs. client-informed brief*
...
![Causal diagram of the fluency trap: a model-output node (statistical coherence, no communicative intent) and a reader-cognition node (processing-fluency reward) reinforce each other through an amplification arc; both feed forward past a bypassed "is it informed?" check shown off the main line with a dashed connector, into a closed-trap terminal state, with a blockage symbol marking where professional judgment can interrupt the chain.](images/01-what-ai-plus-one-is-fig-01.png)
*Figure 1.1 — The fluency trap as a two-component mechanism*
...
The three-LLM domain research prompt is the move that catches what one model misses. You run the same structured prompt across Claude, GPT, and Gemini. Where all three agree, you have settled territory. Where they diverge, you have contested ground worth flagging. Where one raises a point the others missed, you have a candidate for your synthesis. This is investigator triangulation in Norman Denzin's 1978 vocabulary — using multiple independent sources to catch what any single one misses.[^denzin]
<!-- FACT-CHECK FLAG: UNVERIFIED — see factchecks/01-what-ai-plus-one-is-assertions.md -->
...
![Convergence diagram with three source nodes across the top — Claude, GPT, and Gemini — whose arrows flow downward through a three-way Venn-like overlap region. The overlap zones encode the four claim categories: the central triple-overlap is all-three-agree, the pairwise lenses are two-agree, the single-source spurs are one-only, and non-overlap marks divergence. All three feed one synthesis node at the bottom.](images/01-what-ai-plus-one-is-fig-04.png)
...
Use the prompt template from this chapter. Adapt the bracketed field to your profession. Run it verbatim in Claude (with web research enabled), GPT-4 or later, and Gemini. Save all three outputs as plain markdown.
...
Tic TOC is the structural response. The two-hour conversation that produces a TIKTOC.md is the architecture that catches the fluency trap before it becomes a chapter, then a book. Chapter 2 shows you the product — the ai-for-designers TIKTOC.md, in full — before it explains how the product was made.
...
### Figure 1.1 — The fluency trap as a two-component mechanism
Build a single horizontal causal-chain diagram (systems diagram, not a chart) as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: five fixed nodes plus one interruption marker — (1) model output: statistical coherence, no communicative intent; (2) reader cognition: processing-fluency reward; (3) bypassed "is it informed?" check; (4) closed-trap terminal; plus a judgment-interruption blockage. Marks: rounded rectangles for nodes, straight arrows for the main left-to-right chain, a curved feedback arc between nodes 1 and 2 (mutual amplification), a dashed connector to node 3 placed off the main line to encode "skipped," and a blockage glyph (⊣) on the final segment. Channels: x position encodes causal order; the dashed/off-line treatment encodes bypass; color marks only the primary chain (red) versus neutral supporting nodes. No quantitative axis, no zero baseline. Label every node and the interruption point. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

### Figure 1.2 — Generic brief vs. client-informed brief
Build a two-panel comparison diagram as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: two parallel documents, each with six stacked sections; the right document additionally carries six "knowledge gap" markers, one per section. Marks: filled rectangles (section bars) for both panels; on the right panel add six hollow ring markers in the margin connected by thin leader lines to their sections. Channels: y position = section order (shared across panels); fill versus hollow = present versus missing institutional knowledge; left/right column = generic versus client-informed. No quantitative axis. Sort: sections top-to-bottom in document order. Annotate each gap marker with its short label (gradient grudge, PMS green, new VP, no budget, rival shops, six-person committee). Exactly six gaps. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

### Figure 1.3 — Wage premium by sector and skill level
Build a vertical bar chart as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: one series, three bars — PwC US advanced-AI-skills premium 56%, Lightcast AI-posting premium 28%, and a lower contested cut (~14%). Single value channel: percent wage premium, range 0–60. Marks: rectangles. Channels: x = source/cut (band scale), y = percent (linear, zero baseline required). Color: the two confirmed bars in the primary series color; the contested bar in a distinct non-red fill to flag it as uneven. Sort: descending by magnitude. Annotate each bar with its value label and source. No dollar figures mixed onto the percent axis. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.

### Figure 1.4 — Three-LLM triangulation to synthesis
Build a top-to-bottom convergence diagram as one standalone HTML file with inline CSS and D3 7.9.0 from the CDN. Data shape: three source nodes (Claude, GPT, Gemini) feeding four claim-category regions feeding one synthesis node. Marks: three circles arranged as a three-way Venn at the top, arrows flowing downward into a single synthesis rectangle at the bottom. Channels: the overlap geometry encodes the four categories — central triple-overlap = all-three-agree, pairwise lenses = two-agree, single-circle crescents = one-only, and a forked marker beside the cluster = divergent. Color: one distinct hue per source circle, synthesis node in the primary series color; overlap outlines neutral. No quantitative axis, no zero baseline. Label the three sources, the four category regions, and the synthesis node. Deliverable: one self-contained HTML file, inline CSS, D3 7.9.0 CDN, responsive via ResizeObserver, accessible SVG with title/desc.
